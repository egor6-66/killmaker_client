...
export interface IHTMLMediaProps extends React.AudioHTMLAttributes<any>, React.VideoHTMLAttributes<any> {
    src: string;
}

export interface IHTMLMediaState {
    buffered: any[];
    duration: number;
    paused: boolean;
    muted: boolean;
...
export interface IHTMLMediaControls {
...
type MediaPropsWithRef<T> = IHTMLMediaProps & { ref?: React.MutableRefObject<T | null> };

export default function createHTMLMediaHook<T extends HTMLAudioElement | HTMLVideoElement>(tag: 'audio' | 'video') {
    return (elOrProps: IHTMLMediaProps | React.ReactElement<IHTMLMediaProps>) => {
        ...
        const [state, setState] = useSetState<IHTMLMediaState>({
            buffered: [],
            time: 0,
            duration: 0,
            paused: true,
